...
% Author:
%  Zohar Bar-Yehuda
% Modified by Sohrob Saeb to comply with MATLAB R2014bb
%
...
inputParams.axis = axHandle;
inputParams.params = varargin;
...
for idx = 1:length(axes_objs)
    if ~isempty(findobj(get(axes_objs(idx),'children'),'tag','gmap'));
        ...
        if inputParams.axis==axes_objs(idx)
